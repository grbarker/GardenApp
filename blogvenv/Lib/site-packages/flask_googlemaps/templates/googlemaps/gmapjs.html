...
<script type="text/javascript">
    ...
    function initialize_{{gmap.varname}}() {
        ...
        google.maps.event.addListener(
            {{gmap.varname}},
            'click',
            function(event) { clickposCallback2('{{gmap.clickpos_uri}}', event.latLng) }
        );
        {% endif %}
    ...
    function clickposCallback2(uri, latLng) {
        document.getElementById('latspan').innerHTML = latLng.lat()
        document.getElementById('lngspan').innerHTML = latLng.lng()
        $.get(
            "https://maps.googleapis.com/maps/api/geocode/json?latlng=" + latLng.lat() + "," + latLng.lng() + "&key=AIzaSyCyX0uZDxs4ekWQz-uSuhvhpABMOFf8QfI",
            function(result) {
                for (i = 0; i < result.results.length; i++) {
                    var html, address;
                    const uri = "'main.registerGarden'";
                    address = result.results[i].formatted_address;
                    html = "<p><a href='{{ url_for("main.registerGardenFromMap", address="xxxx") }}'>xxxx</a></p><br>".replace(/xxxx/g, address);
                    document.getElementById("addresslist").insertAdjacentHTML("afterbegin", html);
                }
            }
        );
    }
